...
def simulate_closed_loop_dynamics():
  ...
  ax6 = fig.add_subplot(gs[3, 0:3], sharex=ax5)
  ax7 = fig.add_subplot(gs[4, 0:3], sharex=ax5)
  ...
  anim_par = AnimPVTOLAircraftPar(aircraft_size=1.5)
  ...
  animators.append(AnimTrace(ax1, simres.traj.time, simres.traj.coords[:,0], simres.traj.coords[:,1], 4, False, alpha=0.5))

  plt.sca(ax2)
  plt.grid(True)
  add_annotation('$z$', [0.83, 0.83])
  ...
  plt.sca(ax3)
  plt.grid(True)
  add_annotation(R'$\psi$', [0.83, 0.83])
  ...
  plt.sca(ax4)
  plt.grid(True)
  add_annotation(R'$\dot x$', [0.83, 0.83])
  ...
  animators.append(AnimScope(ax5, simres.traj.time, theta, simres.traj.control[:,0], 
                          alpha=0.7, lw=1))
  add_annotation('thrust', [0.85, 0.82], 16)
  ...
  animators.append(AnimScope(ax6, simres.traj.time, theta, simres.traj.control[:,1], 
                          alpha=0.7, lw=1))
  add_annotation('torque', [0.85, 0.82], 16)

  plt.sca(ax7)
  plt.grid(True)
  set_pi_xticks('1', fontsize=16)
  animators.append(AnimScope(ax7, simres.traj.time, theta, xi,  alpha=0.7, lw=1))
  add_annotation('transverse coordinates', [0.65, 0.82], 16)
  plt.xlabel('trajectory projection', fontsize=16)

  plt.tight_layout(h_pad=1, pad=1)
  anim = Animate(fig, animators, simres.traj.time[-1], fps=60, dpi=120, videopath='fig/pvtol-tictoc.mp4')
