...
def get_vis_par(systempar : CartPendulumPar):
  l = systempar.pendulum_length
  return CartPendulumVisPar(
    cart_width = 0.4 * l,
    cart_height = 0.16 * l,
    rod_length = l,
    rod_width = 3e-2 * l,
...
def get_view_box(q : np.ndarray, par : CartPendulumVisPar) -> Rect:
  """
...
class CartPendulumVis:
  def __init__(self, ax : Axes, par : CartPendulumVisPar):
    self.__par = par
    self.__create_body()
